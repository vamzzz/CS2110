...
void* my_malloc(size_t size) {
    ...
    if (smallest_Avail == NULL) {
    	...
    	new_block->size = SBRK_SIZE / WORD_SIZE;
    	new_block->next = NULL;
    	add_to_freelist(new_block);
    	return my_malloc(size);
    } else {
    	//if the block is found - split it if what is left is big enough
    	if (should_split(smallest_Avail, real_size / WORD_SIZE) == 1) {
    		metadata_t* block = split_block(smallest_Avail, real_size / WORD_SIZE);
    		//setCanary(block, real_size / WORD_SIZE);
    		return	block + 1;
    	} else {
    		//since it is not big enough, remove smallest from list
    		setCanary(smallest_Avail, smallest_Avail -> size);
 ...
 */
void* split_block(metadata_t* block, size_t size) {
	metadata_t* new_block = (metadata_t*) (((char*) block) + (block->size - size) * WORD_SIZE);
	new_block->next = NULL;
	new_block->size = size;
 ...
 */
void setCanary(metadata_t* block, size_t request) {
	unsigned long* canLoc = (unsigned long*)((char*)(block) + (request * WORD_SIZE) - sizeof(long));
...
void add_to_freelist(metadata_t* block) {
	...
	if ((uintptr_t) block < (uintptr_t) current) {
		...
		// if ((metadata_t*)((char*)block + (block->size) * WORD_SIZE) == current) {
	...
	while (current->next != NULL) {
		if ((uintptr_t) block < (uintptr_t) current->next) {
			...
			block->next = newB;
			//left merge
			// if ((metadata_t*)((char*)current + (current->size) * WORD_SIZE) == block) {
			// 	current->size = current->size + block->size;
			// 	current->next = block->next;
			// 	block = current;
			// }
			// //right merge
			// if ((metadata_t*) ((char*)block + (block->size) * WORD_SIZE) == newB) {
...
void* my_realloc(void* ptr, size_t size) {
	...
	if (ptr == NULL) {
		...
		if (location->size < size / WORD_SIZE) {			
			memcpy(newpointer, ptr, (location->size) * WORD_SIZE);
			my_free(ptr);
		} else {
...
void merge(metadata_t* block) {
	...
	if (left != NULL) {
		if (block == (metadata_t*) (((char*) left) + ((left -> size) * WORD_SIZE))) {
			left -> size = left -> size + block -> size;
			left -> next = block -> next;
			left -> canary = calculateCanary(left);
			block = left;
		}
	}

	if (right != NULL) {
		if (right == (metadata_t*) (((char*) block) + ((block -> size) * WORD_SIZE))) {
			block -> size = block -> size + right -> size;
			block -> next = right -> next;
			block -> canary = calculateCanary(block);
			right = block;
		}
	}
